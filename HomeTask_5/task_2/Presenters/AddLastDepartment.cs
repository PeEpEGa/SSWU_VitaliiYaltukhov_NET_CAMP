...
{
    internal class AddLastDepartment : IPresenter
    {
        private readonly IDepartmentStoreService _departmentService;

        public AddLastDepartment(IDepartmentStoreService departmentService)
        {
        ...
        public IPresenter Action()
        {
            ...
            try
            {
                ...
                LastDepartment lastDep = new LastDepartment { Name = nameOfDepartment, Products = new List<Item>() };
                printed = AddDepartment(departments.ToList(), departmentToFind, lastDep);
            }
            catch (Exception exc)
            {
            ...
            if (printed)
            {
                Console.WriteLine("Last department added successfully.");
            }
            else
            {
        ...
        private bool AddDepartment(List<Department> departments, string departmentName, LastDepartment lastDepartment)
        {
            bool added = false;
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    possibleDepartment.LastDepartment = lastDepartment;
                    return true;
                }
                var dep = departments[i].Departments;
                added = AddDepartment(dep, departmentName, lastDepartment);
            }
            return added;
        }
    }
